    ...
    const scheduleNextExecution = () => {
        ...
        setTimeout(async () => {
            try {
                ...
                await user.send(
                    `Привет! На этой неделе было загружено ${fileCount} изображений. Архивы прикреплены.`
                );
                await user.send({ files: archivePaths });

                // Очищаем папку и удаляем архивы
                clearDirectory(sourceDir);
                archivePaths.forEach((archPath) => fs.unlinkSync(archPath));
...
export async function DanteInformator(message: Message) {
    const version = '1.2.14';
    const lastUpdate = '2024-12-09';
    ...
    const embed = new EmbedBuilder()
        ...
        const avatar = new AttachmentBuilder('assets/botAvatar.png');

    if (message.channel instanceof TextChannel || message.channel instanceof DMChannel || message.channel instanceof NewsChannel) {
        await message.channel.send({ embeds: [embed], files:[avatar] });
    } else {
...
export function startScheduledMessage(
    ...
    setInterval(() => {
        ...
        const newMessage = `Осталось времени до следуюшего гег-сообшения - ${totalHours.toString().padStart(2, '0')}ч ${minutes.toString().padStart(2, '0')}м ${seconds.toString().padStart(2, '0')}с ${spinnerFrames[spinnerIndex]}`;
